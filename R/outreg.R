...
outreg <- function(incoming, title="My Regression", label="",
                   modelLabels=NULL, varLabels=NULL,
                   exclude,
                   tight=TRUE, showAIC=TRUE, lyx=TRUE)
{
    
    modelList <- NULL
    
    ## was input just one model, or a list of models?  ###
    if ( "reg" %in% class(incoming)) { ##just one model input
        nmodels <- 1
  ...
  for (model in modelList){
    ...
    if(missing(exclude))
        excl <- 1:length(fixnames)
    else
        excl <- -grep(exclude, fixnames)
